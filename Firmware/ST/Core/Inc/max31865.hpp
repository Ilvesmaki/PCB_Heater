...
private:
    ...
    uint16_t _cs_pin;
    SPI_HandleTypeDef *_spi_handler = nullptr;
    
    /* private functions */
    float calculate_temp_c(float rtd_zero_c);
public:
    uint8_t read_reg(const uint8_t &addr);
    void write_reg(const uint8_t &addr, const uint8_t &data);

    max31865(SPI_HandleTypeDef *spi, GPIO_TypeDef *cs_port, uint16_t cs_pin);
    ~max31865();
    
    void set_config(const max31865_config_t &config);
    void set_resistors(const uint16_t ref, max31865_rtd_type_t rtd);
